        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            <Link href="/" className="flex-shrink-0 flex items-center">
              <span className="font-bold text-2xl tracking-tighter uppercase text-black">YAZZED <span className="text-gray-400">SHOP MART</span><span className="text-gray-500">.</span></span>
            </Link>
            <div className="hidden md:ml-10 md:flex md:space-x-8">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.path}
                  className="text-gray-600 inline-flex items-center px-1 pt-1 text-sm font-medium hover:text-black transition-colors"
                >
                  {link.name}
                </Link>
              ))}
          ...
          <div className="hidden md:flex items-center space-x-6">
            {mounted && isAuthenticated ? (
              ...
              <div className="flex space-x-4">
                 <Link href="/login" className="text-gray-600 text-sm font-medium hover:underline">
                   Login
                 </Link>
